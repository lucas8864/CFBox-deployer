...
async function deploy(d, request) {
  ...
  const verification = type === "pages" ? await verifyPages(t, a, name, uuid || null, kv?.id || null, domain2 || null, log3) : await verifyWorker(t, a, name, uuid || null, kv?.id || null, domain2 || null, log3);
  const failedChecks = Object.entries(verification || {}).filter(([k, v]) => v === false).map(([k]) => k);
  if (failedChecks.length) log3(`\u26A0 \u90E8\u7F72\u8BF7\u6C42\u5B8C\u6210\uFF0C\u4F46\u4EE5\u4E0B\u9A8C\u8BC1\u672A\u901A\u8FC7: ${failedChecks.join(", ")}`);
  else log3(`${mode === "create" ? "\u90E8\u7F72" : "\u66F4\u65B0"}\u5B8C\u6210\uFF0C\u5168\u90E8\u5173\u952E\u9A8C\u8BC1\u901A\u8FC7`);
  return {
    projectName: name,
    deployType: type,
    uuid: uuid || null,
...
async function bindPagesDomain(t, a, n, domain2, log3) {
  ...
  try {
    ...
    log3(`\u68C0\u67E5 Pages \u81EA\u5B9A\u4E49\u57DF\u540D\u5931\u8D25: ${e.message}`);
  }
  if (!old) {
    ...
    log3(`Pages \u81EA\u5B9A\u4E49\u57DF\u540D\u5DF2\u63D0\u4EA4: ${d}`);
  } else {
    log3(`Pages \u81EA\u5B9A\u4E49\u57DF\u540D\u5DF2\u5B58\u5728: ${d}`);
  }
  const dns = await ensurePagesDnsRecord(t, zone, d, `${n}.pages.dev`, log3);
  return { name: d, zoneId: zone.id, dns, status: old?.status || old?.verification_status || "pending" };
}
__name(bindPagesDomain, "bindPagesDomain");
async function ensurePagesDnsRecord(t, zone, domain2, target, log3) {
  const records = await listDnsExact(t, zone.id, domain2);
  let record = records.find((r) => r.type === "CNAME");
  if (record) {
    log3(`Pages DNS \u5DF2\u5B58\u5728: CNAME ${domain2} -> ${record.content}`);
    return record;
  }
  const conflict = records.find((r) => ["A", "AAAA"].includes(r.type));
  if (conflict) throw Error(`Pages DNS \u5DF2\u5B58\u5728 ${conflict.type} \u8BB0\u5F55: ${domain2} -> ${conflict.content}\uFF0C\u672A\u8986\u76D6\u73B0\u6709\u4E1A\u52A1`);
  record = await cf(t, `/zones/${zone.id}/dns_records`, {
    method: "POST",
    body: { type: "CNAME", name: domain2, content: target, ttl: 1, proxied: true }
  });
  log3(`Pages DNS \u5DF2\u521B\u5EFA: CNAME ${domain2} -> ${target}`);
  return record;
}
__name(ensurePagesDnsRecord, "ensurePagesDnsRecord");
async function bindWorkerDomain(t, a, n, domain2, log3) {
  const d = normalizeDomain(domain2);
  const zone = await findZoneForDomain(t, d);
  if (!zone) throw Error(`\u672A\u627E\u5230\u53EF\u7ED1\u5B9A\u7684 Cloudflare Zone: ${d}`);
  const dns = await ensureWorkerDnsRecord(t, zone, d, log3);
  ...
  if (old?.id) {
    ...
    log3(`Worker Route \u5DF2\u66F4\u65B0: ${pattern} -> ${n}`);
  } else {
    await cf(t, `/zones/${zone.id}/workers/routes`, { method: "POST", body });
    log3(`Worker Route \u5DF2\u521B\u5EFA: ${pattern} -> ${n}`);
  }
  return { name: d, zoneId: zone.id, dns };
}
__name(bindWorkerDomain, "bindWorkerDomain");
async function findZoneForDomain(t, domain2) {
  const zones = await cf(t, "/zones?per_page=100&status=active");
  return zones.filter((z) => domain2 === z.name || domain2.endsWith(`.${z.name}`)).sort((x, y) => y.name.length - x.name.length)[0] || null;
}
__name(findZoneForDomain, "findZoneForDomain");
async function listDnsExact(t, zoneId, domain2) {
  const records = await cf(t, `/zones/${zoneId}/dns_records?name=${encodeURIComponent(domain2)}&per_page=100`);
  return Array.isArray(records) ? records : [];
}
__name(listDnsExact, "listDnsExact");
async function ensureWorkerDnsRecord(t, zone, domain2, log3) {
  const records = await listDnsExact(t, zone.id, domain2);
  const conflict = records.find((r) => r.type === "CNAME");
  if (conflict) {
    throw Error(`\u65E0\u6CD5\u521B\u5EFA Worker Route DNS\uFF1A${domain2} \u5DF2\u5B58\u5728 CNAME \u8BB0\u5F55\uFF0C\u8BF7\u5148\u5220\u9664\u6216\u6539\u7528\u5176\u4ED6\u5B50\u57DF\u540D`);
  }
  ...
  if (record) {
    if (!record.proxied) {
      record = await cf(t, `/zones/${zone.id}/dns_records/${record.id}`, {
        method: "PUT",
        body: { type: "AAAA", name: domain2, content: "100::", ttl: 1, proxied: true }
      });
      log3(`Worker DNS AAAA \u5DF2\u66F4\u65B0\u4E3A\u4EE3\u7406\u72B6\u6001: ${domain2} -> 100::`);
    } else {
      log3(`Worker DNS \u5DF2\u5B58\u5728: AAAA ${domain2} -> 100::\uFF08\u5DF2\u4EE3\u7406\uFF09`);
    }
    return record;
  }
  const real = records.find((r) => ["A", "AAAA"].includes(r.type));
  if (real) {
    throw Error(`DNS \u5DF2\u5B58\u5728 ${real.type} \u8BB0\u5F55: ${domain2} -> ${real.content}\uFF0C\u4E3A\u907F\u514D\u8986\u76D6\u73B0\u6709\u4E1A\u52A1\uFF0C\u672C\u6B21\u672A\u4FEE\u6539`);
  }
  record = await cf(t, `/zones/${zone.id}/dns_records`, {
    method: "POST",
    body: { type: "AAAA", name: domain2, content: "100::", ttl: 1, proxied: true }
  });
  log3(`Worker DNS \u5DF2\u521B\u5EFA: AAAA ${domain2} -> 100::\uFF08Cloudflare \u4EE3\u7406\uFF09`);
  return record;
}
__name(ensureWorkerDnsRecord, "ensureWorkerDnsRecord");
...
async function verifyWorker(t, a, n, uuid, kvId, domain2, log3) {
  const result = { project: false, uuid: uuid ? false : null, kv: kvId ? false : null, domain: domain2 ? false : null, dns: domain2 ? false : null, workersDev: false };
...
async function verifyPages(t, a, n, uuid, kvId, domain2, log3) {
  const result = { project: false, uuid: uuid ? false : null, kv: kvId ? false : null, domain: domain2 ? false : null, dns: domain2 ? false : null };
  ...
  if (domain2) {
    ...
    const zone = await findZoneForDomain(t, domain2);
    if (zone) {
      const dns = await listDnsExact(t, zone.id, domain2);
...
var worker_default = {
  async fetch(request, env2, ctx) {
    const url = new URL(request.url);
    if (url.pathname.startsWith("/api/")) return onRequest({ request, env: env2, ctx, params: {} });
...
//# sourceMappingURL=bundledWorker-0.1748797133379847.mjs.map
